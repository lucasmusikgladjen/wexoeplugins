 ...
 * Description: Modular, high-converting landing pages driven by Airtable data. Use [wexoe_landing slug="fjarraccess"] to render.
 * Version: 1.0.0
 * Author: Wexoe
...
if (!defined('WEXOE_LP_AIRTABLE_API_KEY')) define('WEXOE_LP_AIRTABLE_API_KEY', '.');
if (!defined('WEXOE_LP_AIRTABLE_BASE_ID')) define('WEXOE_LP_AIRTABLE_BASE_ID', 'appXoUcK68dQwASjF');
if (!defined('WEXOE_LP_TABLE_PAGES'))      define('WEXOE_LP_TABLE_PAGES', 'Landing Pages');
if (!defined('WEXOE_LP_TABLE_TABS'))       define('WEXOE_LP_TABLE_TABS', 'LP Tabs');
if (!defined('WEXOE_LP_TABLE_DOWNLOADS'))  define('WEXOE_LP_TABLE_DOWNLOADS', 'LP Downloads');
if (!defined('WEXOE_LP_CACHE_TTL'))        define('WEXOE_LP_CACHE_TTL', 300);
...
if (!function_exists('wexoe_lp_airtable_request')) {
function wexoe_lp_airtable_request($table, $params = []) {
    $url = 'https://api.airtable.com/v0/' . WEXOE_LP_AIRTABLE_BASE_ID . '/' . rawurlencode($table);
    if (!empty($params)) $url .= '?' . http_build_query($params);
    $response = wp_remote_get($url, [
        'headers' => ['Authorization' => 'Bearer ' . WEXOE_LP_AIRTABLE_API_KEY],
        'timeout' => 15,
    ]);
...
function wexoe_lp_fetch_page($slug) {
    $cache_key = 'wexoe_lp_page_' . sanitize_key($slug);
    $cached = get_transient($cache_key);
    if ($cached !== false) return $cached;

    $result = wexoe_lp_airtable_request(WEXOE_LP_TABLE_PAGES, [
        'filterByFormula' => '{Slug}="' . addslashes($slug) . '"',
        'maxRecords' => 1,
    ]);

    if (isset($result['error']) || empty($result['records'])) return null;

    $data = $result['records'][0]['fields'];
    $data['_record_id'] = $result['records'][0]['id'];
    set_transient($cache_key, $data, WEXOE_LP_CACHE_TTL);
    return $data;
}

function wexoe_lp_fetch_linked($table, $record_ids, $prefix) {
    if (empty($record_ids)) return [];
    $cache_key = 'wexoe_lp_' . $prefix . '_' . md5(implode(',', $record_ids));
    $cached = get_transient($cache_key);
    if ($cached !== false) return $cached;

    $parts = [];
    foreach ($record_ids as $rid) $parts[] = 'RECORD_ID()="' . $rid . '"';
    ...
    do {
        ...
        $result = wexoe_lp_airtable_request($table, $params);
        if (isset($result['error']) || !isset($result['records'])) break;
        foreach ($result['records'] as $rec) {
            $f = $rec['fields']; $f['_record_id'] = $rec['id']; $all[] = $f;
        }
        $offset = isset($result['offset']) ? $result['offset'] : null;
    } while ($offset);

    usort($all, function($a, $b) {
        return (isset($a['Order']) ? (float)$a['Order'] : 999) - (isset($b['Order']) ? (float)$b['Order'] : 999);
    });
    $all = array_values(array_filter($all, function($r) { return !empty($r['Visa']); }));

    set_transient($cache_key, $all, WEXOE_LP_CACHE_TTL);
    return $all;
}
...
function wexoe_lp_render_hero($data, $id) {
    ...
    $desc = wexoe_lp_field($data, 'Hero Description', '');
    $image = wexoe_lp_field($data, 'Hero Image', '');
    $cta1_text = wexoe_lp_field($data, 'Hero CTA Text', 'Kontakta oss');
    $cta1_url = wexoe_lp_field($data, 'Hero CTA URL', '/kontakt/');
    $cta2_text = wexoe_lp_field($data, 'Hero CTA2 Text', '');
    $cta2_url = wexoe_lp_field($data, 'Hero CTA2 URL', '');
...
function wexoe_lp_render_content_sidebar($data, $id) {
    $show_content = wexoe_lp_field($data, 'Show Content', true);
    $show_sidebar = wexoe_lp_field($data, 'Show Sidebar', true);
    $sidebar_type = wexoe_lp_field($data, 'Sidebar Type', '');

    $h2 = wexoe_lp_field($data, 'Content H2', '');
    $text = wexoe_lp_field($data, 'Content Text', '');
    $benefits = wexoe_lp_lines_to_array(wexoe_lp_field($data, 'Content Benefits', ''));
...
function wexoe_lp_render_sidebar_case($data, $id) {
    $title = wexoe_lp_field($data, 'Case Title', '');
    $desc = wexoe_lp_field($data, 'Case Description', '');
    $image = wexoe_lp_field($data, 'Case Image', '');
    $outcomes = wexoe_lp_lines_to_array(wexoe_lp_field($data, 'Case Outcomes', ''));
    $cta_text = wexoe_lp_field($data, 'Case CTA Text', '');
    $cta_url = wexoe_lp_field($data, 'Case CTA URL', '');
...
function wexoe_lp_render_sidebar_calculator($data, $id) {
    $title = wexoe_lp_field($data, 'Calc Title', '');
    $calc_html = wexoe_lp_field($data, 'Calc HTML', '');
...
function wexoe_lp_render_sidebar_event($data, $id) {
    $type = wexoe_lp_field($data, 'Event Type', '');
    $title = wexoe_lp_field($data, 'Event Title', '');
    $desc = wexoe_lp_field($data, 'Event Description', '');
    $date = wexoe_lp_field($data, 'Event Date', '');
    $location = wexoe_lp_field($data, 'Event Location', '');
    $webhook = wexoe_lp_field($data, 'Event Webhook', '');
...
function wexoe_lp_render_sidebar_leadmagnet($data, $id) {
    $title = wexoe_lp_field($data, 'Magnet Title', '');
    $format = wexoe_lp_field($data, 'Magnet Format', '');
    $desc = wexoe_lp_field($data, 'Magnet Description', '');
    $file_url = wexoe_lp_field($data, 'Magnet File URL', '');
    $webhook = wexoe_lp_field($data, 'Magnet Webhook', '');
...
function wexoe_lp_render_tabs($tabs, $downloads_map, $data, $id) {
    ...
    $show = wexoe_lp_field($data, 'Show Tabs', true);
    if (!$show) return '';
    ...
    foreach ($tabs as $i => $tab) {
        ...
        $type = wexoe_lp_field($tab, 'Tab Type', '');
        ...
        switch ($type) {
            ...
            case 'downloads':
                $dl_ids = wexoe_lp_get_ids($tab, 'LP Downloads');
...
function wexoe_lp_render_tab_textimage($tab, $id) {
    $h2 = wexoe_lp_field($tab, 'TI H2', '');
    $text = wexoe_lp_field($tab, 'TI Text', '');
    $benefits = wexoe_lp_lines_to_array(wexoe_lp_field($tab, 'TI Benefits', ''));
    $image = wexoe_lp_field($tab, 'TI Image', '');
    $inverted = !empty($tab['TI Inverted']);
...
function wexoe_lp_render_tab_faq($tab, $id) {
    $faqs = wexoe_lp_parse_faq(wexoe_lp_field($tab, 'FAQ Items', ''));
...
function wexoe_lp_render_tab_calameo($tab, $id) {
    ...
    for ($n = 1; $n <= 3; $n++) {
        $title = wexoe_lp_field($tab, 'Calameo '.$n.' Title', '');
        $src = wexoe_lp_field($tab, 'Calameo '.$n.' Src', '');
...
function wexoe_lp_render_tab_downloads($tab, $downloads, $id) {
    ...
    foreach ($downloads as $dl) {
        ...
        $file = wexoe_lp_field($dl, 'File URL', '');
        $btn = wexoe_lp_field($dl, 'Button Text', 'Ladda ner PDF');
...
function wexoe_lp_render_tab_compare($tab, $id) {
    $title = wexoe_lp_field($tab, 'Compare Title', '');
    $col_a = wexoe_lp_field($tab, 'Compare Col A', 'A');
    $col_b = wexoe_lp_field($tab, 'Compare Col B', 'B');
    $rows = wexoe_lp_parse_compare_rows(wexoe_lp_field($tab, 'Compare Rows', ''));
...
function wexoe_lp_render_tab_steps($tab, $id) {
    $title = wexoe_lp_field($tab, 'Steps Title', '');
...
function wexoe_lp_render_contact($data, $id) {
    $name = wexoe_lp_field($data, 'Contact Name', '');
    if (empty($name)) return '';
    $show = wexoe_lp_field($data, 'Show Contact', true);
    if (!$show) return '';

    $title = wexoe_lp_field($data, 'Contact Title', '');
    $email = wexoe_lp_field($data, 'Contact Email', '');
    $phone = wexoe_lp_field($data, 'Contact Phone', '');
    $image = wexoe_lp_field($data, 'Contact Image', '');
    $quote = wexoe_lp_field($data, 'Contact Quote', '');
...
function wexoe_landing_page_shortcode($atts) {
    ...
    if ($atts['nocache'] === 'true') {
        global $wpdb;
        $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_wexoe_lp_%' OR option_name LIKE '_transient_timeout_wexoe_lp_%'");
    }
    ...
    // Fetch linked tabs
    $tab_ids = wexoe_lp_get_ids($data, 'LP Tabs');
    $tabs = wexoe_lp_fetch_linked(WEXOE_LP_TABLE_TABS, $tab_ids, 'tabs');

    // Fetch downloads for tabs that need them
    $downloads_map = [];
    foreach ($tabs as $tab) {
        if (wexoe_lp_field($tab, 'Tab Type', '') === 'downloads') {
            $dl_ids = wexoe_lp_get_ids($tab, 'LP Downloads');
            if (!empty($dl_ids)) {
                $dls = wexoe_lp_fetch_linked(WEXOE_LP_TABLE_DOWNLOADS, $dl_ids, 'dl_' . $tab['_record_id']);
                $downloads_map[$tab['_record_id']] = $dls;
            }
        }
    }

    // Colors
    $main_color = wexoe_lp_hex(wexoe_lp_field($data, 'Color Main', ''), '#11325D');
    $secondary_color = wexoe_lp_hex(wexoe_lp_field($data, 'Color Secondary', ''), '#F28C28');
