...
interface Props { state: UniquePageState; }
 ...
 */
export default function UniquePagePreview({ state }: Props) {
  return (
    <div className="h-full overflow-y-auto bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto bg-white shadow-sm">
        ...
        {/* Hero */}
        {state.showHero && (
          <SectionFrame label="Hero" theme={state.hero.theme}>
        ...
        {state.showTextImageA && <TextImageBlock label="Text+bild A" h2={state.textImageA.h2} body={state.textImageA.body} theme={state.textImageA.theme} reversed={state.textImageA.reversed} image={state.textImageA.imageUrl} />}
        {state.showTextImageB && <TextImageBlock label="Text+bild B" h2={state.textImageB.h2} body={state.textImageB.body} theme={state.textImageB.theme} reversed={state.textImageB.reversed} image={state.textImageB.imageUrl} />}

        {state.showTextOnly && (
          <SectionFrame label="Text" theme="light">
        ...
        {state.showFaq && (
          <SectionFrame label="FAQ" theme="light">
        ...
        {state.showTeamGrid && (
          <SectionFrame label="Team grid (SSOT)" theme="light">
        ...
        {state.showPartnersMarquee && (
          <SectionFrame label="Partners marquee (SSOT)" theme="light">
        ...
        {state.showTestimonialCard && (
          <SectionFrame label="Citat-kort (SSOT)" theme="light">
            <p className="text-xs text-gray-400">
        ...
        {state.showCtaBanner && (
          <SectionFrame label="CTA-banner" theme={state.ctaBanner.theme}>
...
function SectionFrame({ label, theme, children }: { label: string; theme: 'dark' | 'light'; children: React.ReactNode }) {
...
function TextImageBlock({ label, h2, body, theme, reversed, image }: { label: string; h2: string; body: string; theme: 'dark' | 'light'; reversed: boolean; image: string }) {
  return (
    <SectionFrame label={label} theme={theme}>
