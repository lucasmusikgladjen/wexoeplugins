...
function Eyebrow({ text, light = false, bookend = false }: { text: string; light?: boolean; bookend?: boolean }) {
  ...
  const Dash = () => (
    <span style={{ display: 'inline-block', flexShrink: 0, width: 22, height: 1, background: color, verticalAlign: 'middle', margin: '0 10px' }} />
  );
  return (
...
export function CtaBannerPreview({ section }: { section: CtaBannerSection }) {
  return (
    <div style={{ background: C.paper, ...sectionPad, fontFamily: font }}>
      <div style={innerMax}>
        <div style={{ background: navyGradient, padding: '56px 48px', borderRadius: 2, boxShadow: '0 24px 60px rgba(10,26,46,0.18)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            {section.body && (
              <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.80)', lineHeight: 1.7, maxWidth: 560, margin: '0 auto' }}>
                {section.body}
              </p>
            )}
            <ButtonRow cta={{ text: section.ctaText, url: section.ctaUrl }} cta2={{ text: section.cta2Text, url: section.cta2Url }} light centered />
          </div>
        </div>
      </div>
    </div>
