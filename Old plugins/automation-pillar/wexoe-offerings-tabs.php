 ...
 * Description: Tabbad sektion för kundtyper (OEM, Integratörer, Slutkunder) med data från Airtable
 * Version: 1.0.0
 * Author: Wexoe
 */
...
class Wexoe_Offerings_Tabs {
    ...
    private $table_id = 'tbldQZJu3NHHP5dUh';
     ...
     */
    private function fetch_airtable_data($division = '') {
        $url = "https://api.airtable.com/v0/{$this->base_id}/{$this->table_id}";
        
        // Lägg till filter om division anges
        if (!empty($division)) {
            $formula = urlencode("{Division} = '{$division}'");
    ...
    public function render_shortcode($atts) {
        ...
        $records = $this->fetch_airtable_data($atts['division']);
        ...
        usort($records, function($a, $b) {
            $order_a = isset($a['fields']['Order']) ? intval($a['fields']['Order']) : 999;
            $order_b = isset($b['fields']['Order']) ? intval($b['fields']['Order']) : 999;
    ...
    private function render_html($id, $records, $title = '') {
        ...
        foreach ($records as $record) {
            ...
            $name = isset($fields['Name']) ? $fields['Name'] : 'Untitled';
        ...
        foreach ($records as $record) {
            ...
            if (isset($fields['Image'])) {
                if (is_array($fields['Image']) && !empty($fields['Image'])) {
                    // Airtable attachment format
                    $image_url = $fields['Image'][0]['url'];
                } elseif (is_string($fields['Image']) && !empty($fields['Image'])) {
                    // Direct URL string
            ...
            $benefits = array();
            for ($i = 1; $i <= 5; $i++) {
                $benefit_key = "Benefit {$i}";
                if (isset($fields[$benefit_key]) && !empty($fields[$benefit_key])) {
                    $benefits[] = $fields[$benefit_key];
                }
            }
            
            // CTA
            $cta_text = isset($fields['CTA Text']) ? $fields['CTA Text'] : 'Kontakta oss';
            $cta_url = isset($fields['CTA URL']) ? $fields['CTA URL'] : '#kontakt';
