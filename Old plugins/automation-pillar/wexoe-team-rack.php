 ...
 * Description: Visar teammedlemmar som CompactLogix I/O-moduler från Airtable
 * Version: 1.1.2
 * Author: Wexoe
 */
...
class WexoeTeamRack {
    ...
    private $airtable_table_id = 'tblldarIcIpxlZ9GV';
    ...
    private function get_team_members($tag = null) {
        if (empty($this->airtable_api_key) || empty($this->airtable_base_id)) {
            return array();
        }
        
        $url = "https://api.airtable.com/v0/{$this->airtable_base_id}/{$this->airtable_table_id}";
        
        // Filtrera på Visa = true
        $filter = "AND({Visa}=TRUE())";
        if ($tag) {
            $filter = "AND({Visa}=TRUE(), FIND('{$tag}', ARRAYJOIN({Tags}, ',')))";
        }
        ...
        foreach ($body['records'] as $record) {
            ...
            $members[] = array(
                ...
                'email' => $fields['Email'] ?? '',
                'phone' => $fields['Phone'] ?? '',
                'tags' => $fields['Tags'] ?? array(),
                'responsibility' => $fields['Responsibility'] ?? array(),
                'module_name' => $fields['Module name'] ?? '',
                'module_color' => $fields['ModuleColor'] ?? '',
    ...
    public function render_shortcode($atts) {
        ...
        if ($atts['debug'] === 'true') {
            ...
            // API Keys
            $debug_output .= '<strong>API KEY:</strong> ' . (empty($this->airtable_api_key) ? '❌ SAKNAS' : '✅ ' . substr($this->airtable_api_key, 0, 15) . '...') . '<br>';
            $debug_output .= '<strong>BASE ID:</strong> ' . (empty($this->airtable_base_id) ? '❌ SAKNAS' : '✅ ' . $this->airtable_base_id) . '<br>';
            $debug_output .= '<strong>TABLE ID:</strong> ' . $this->airtable_table_id . '<br><br>';
            
            if (!empty($this->airtable_api_key) && !empty($this->airtable_base_id)) {
                // Hämta RAW data från Airtable
                $url = "https://api.airtable.com/v0/{$this->airtable_base_id}/{$this->airtable_table_id}";
                $url .= "?filterByFormula=" . urlencode("AND({Visa}=TRUE())");
                $url .= "&sort[0][field]=Order&sort[0][direction]=asc";
                ...
                if (!is_wp_error($response)) {
                    $body = json_decode(wp_remote_retrieve_body($response), true);
                    $debug_output .= '<strong>RAW AIRTABLE FÄLTNAMN (första record):</strong><br>';
                    if (!empty($body['records'][0]['fields'])) {
                        $field_names = array_keys($body['records'][0]['fields']);
                        $debug_output .= '<code style="background:#333;padding:5px;display:block;margin:5px 0;">' . implode(', ', $field_names) . '</code><br>';
                        
                        // Visa specifikt Responsibility-fältet
                        $debug_output .= '<br><strong>RESPONSIBILITY-DATA PER PERSON:</strong><br>';
                        foreach ($body['records'] as $record) {
                            $name = $record['fields']['Name'] ?? 'Okänd';
                            $resp = $record['fields']['Responsibility'] ?? 'SAKNAS';
                            $debug_output .= $name . ': <code style="background:#333;padding:2px 5px;">' . htmlspecialchars(print_r($resp, true)) . '</code><br>';
                        }
                    }
                }
                ...
                if (!empty($members)) {
                    ...
                    $debug_output .= '⚠️ Inga members hittades. Kontrollera att Visa=TRUE i Airtable.<br>';
                }
            } else {
                $debug_output .= '❌ Lägg till i wp-config.php:<br>';
                $debug_output .= '<code style="background:#333;padding:5px;">define(\'AIRTABLE_API_KEY\', \'pat...\');</code><br>';
                $debug_output .= '<code style="background:#333;padding:5px;">define(\'AIRTABLE_BASE_ID\', \'app...\');</code><br>';
            }
        ...
        // Kolla om API-nycklar finns
        if (empty($this->airtable_api_key) || empty($this->airtable_base_id)) {
            if (current_user_can('manage_options')) {
                return '<div style="background:#fee;border:1px solid #c00;padding:15px;margin:20px 0;border-radius:4px;">
                    <strong>⚠️ Wexoe Team Rack:</strong> API-nycklar saknas i wp-config.php<br>
                    <code>define(\'AIRTABLE_API_KEY\', \'...\');</code><br>
                    <code>define(\'AIRTABLE_BASE_ID\', \'...\');</code>
                </div>';
            }
            return '';
        }
