 ...
 * Description: Dynamic hero + value section for audience landing pages. Data from Airtable.
 * Version: 1.0.0
 * Author: Wexoe
 */
...
class Wexoe_Audience_Hero {
    ...
    private $api_key = '.';
    private $base_id = 'appXoUcK68dQwASjF';
    private $table_id = 'tblvNf1CqAYEFvTpu';
     ...
     */
    private function fetch_audience_data($slug) {
        $url = "https://api.airtable.com/v0/{$this->base_id}/{$this->table_id}";
        $url .= "?filterByFormula=" . urlencode("AND({Slug}='{$slug}', {Active}=TRUE())");
    ...
    public function render_shortcode($atts) {
        ...
        $data = $this->fetch_audience_data($atts['slug']);
        ...
        $cta_text = esc_html($data['CTA Text'] ?? 'Kontakta oss');
        $cta_url = esc_url($data['CTA URL'] ?? '/kontakt');
        $hero_image = esc_url($data['Hero Image'] ?? '');
        $stat_number = intval($data['Stat Number'] ?? 0);
        $stat_label = esc_html($data['Stat Label'] ?? '');
        
        $value_h2 = esc_html($data['Value H2'] ?? '');
        $value_text_1 = $this->parse_rich_text($data['Value Text 1'] ?? '');
        $value_text_2 = $this->parse_rich_text($data['Value Text 2'] ?? '');
        $benefit_1 = $this->parse_benefit_formatting(esc_html($data['Benefit 1'] ?? ''));
        $benefit_2 = $this->parse_benefit_formatting(esc_html($data['Benefit 2'] ?? ''));
        $benefit_3 = $this->parse_benefit_formatting(esc_html($data['Benefit 3'] ?? ''));
        
        $case_title = esc_html($data['Case Title'] ?? '');
        $case_description = esc_html($data['Case Description'] ?? '');
        $case_result = esc_html($data['Case Result'] ?? '');
        $case_link_text = esc_html($data['Case Link Text'] ?? 'Läs mer');
        $case_link_url = esc_url($data['Case Link URL'] ?? '#');
